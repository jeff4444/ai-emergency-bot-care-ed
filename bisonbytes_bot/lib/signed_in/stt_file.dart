...
class _SpeechToTextPageState extends State<SpeechToTextPage> {
  ...
  Future<String> _sendMessage(String message) async {
    ...
    String apiKey = 'c9624f8755c04b728722ab27cf4f18ea';
    String deployment_id = 'ai-bot';
    String search_endpoint =
        'https://flyingbisons-bot-ai-search.search.windows.net';
    String search_index_name = 'flying-bisons-idx';
    String search_key = '3WWNcQ2Aa4IkIqBKJWgS1tivcJOmaqTZrTF5TkN7WcAzSeB3Gcvs';
    String apiVersion = '2023-08-01-preview';

    final response = await http.post(
      Uri.parse(
          'https://bot-ai.openai.azure.com/openai/deployments/$deployment_id/extensions/chat/completions?api-version=$apiVersion'),
      headers: {
    ...
    if (response.statusCode == 200) {
      setState(() {
        _messages.addAll(jsonDecode(response.body)['messages']);
      });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Start talking'),
      ),
  ...
  void stopListening() async {
    _logEvent('stop');
    speech.stop();
    print(await _sendMessage(lastWords));
    setState(() {
      level = 0.0;
    });
  }
